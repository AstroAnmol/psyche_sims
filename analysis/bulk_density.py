...
def percentage_of_each_type(df):
    """Returns a dictionary with the count of each particle type in the DataFrame without floor particles."""
...
def total_mass(df):
    """Calculates the total mass from the DataFrame removing the floor particles."""
    unique_types = df['type'].nunique()
